...
export default function BotHistoryPage() {
  ...
  const { data: bots = [] } = useQuery({ queryKey: qk.bots, queryFn: () => api.bots.list() });
  ...
  const filtered = useMemo(
    () =>
      history.filter((h) => {
        ...
        if (botFilter !== "ALL" && String(h.botId) !== botFilter) return false;
  ...
  const totalPnl = filtered.reduce((s, t) => s + Number(t.profit), 0);
  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      <Link href="/bot-trading" className="inline-flex items-center gap-1.5 text-[13px] font-medium text-secondary hover:text-brand-red">
        <ArrowLeft className="h-4 w-4" /> Back to Bot Trading
      </Link>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Win Rate" value={`${winRate}%`} icon={Percent} accent={winRate >= 50 ? "text-emerald-500" : "text-red-500"} />
      </div>
        ...
        {(["ALL", "WIN", "LOSS"] as const).map((r) => (
          <button
            ...
            className={`rounded-lg px-4 py-2 text-[12px] font-semibold ${
              resultFilter === r ? "bg-brand-red text-white" : "border bg-card text-secondary hover:border-brand-red hover:text-brand-red"
            }`}
          >
        ...
        <div className="mt-3 grid gap-3 rounded-2xl border bg-card p-4 shadow-sm sm:grid-cols-2">
          <div>
            ...
            >
              ...
              {bots.map((b) => (<option key={b.id} value={String(b.id)}>{b.name}</option>))}
            </select>
          </div>
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border bg-elevated px-3 py-2 text-[13px] text-primary focus:border-brand-red focus:outline-none"
            >
              {allPairs.map((p) => (<option key={p} value={p}>{p === "ALL" ? "All Pairs" : p}</option>))}
            </select>
          </div>
        </div>
      )}
                ...
                return (
                  <tr key={t.id} className="border-b last:border-none hover:bg-elevated/60">
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className={`inline-flex items-center gap-1 text-[12px] font-semibold ${t.direction === "RISE" ? "text-emerald-500" : "text-red-500"}`}>
                        {t.direction === "RISE" ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                        {t.direction}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right text-secondary">{fmtUsd(Number(t.amount))}</td>
                    <td className="px-4 py-3 text-right">
                      <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${t.result === "WIN" ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"}`}>
                        {t.result}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right text-muted">{new Date(t.tradedAt).toLocaleString()}</td>
                  </tr>
                );
              })}
            </tbody>
          ...
          <div className="flex items-center justify-between border-t bg-elevated px-4 py-3 text-[12px]">
            <p className="text-muted">
              ...
              <span className={totalPnl >= 0 ? "font-semibold text-emerald-500" : "font-semibold text-red-500"}>
                {totalPnl >= 0 ? "+" : ""}{fmtUsd(Math.round(totalPnl))}
              </span>
            </p>
