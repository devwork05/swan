const FEATURES = [
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#000d22" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M7 17L17 7" />
        <path d="M9 7h8v8" />
      </svg>
    ),
  ...
  return (
    <section className="border-t border-brand-border/70 bg-white dark:bg-card">
      <div className="mx-auto grid max-w-[1280px] gap-10 px-5 py-16 sm:grid-cols-2 lg:grid-cols-4 lg:px-8 lg:py-20">
        {FEATURES.map((f) => (
          <div key={f.title}>
            <div className="flex h-[46px] w-[46px] items-center justify-center rounded-[6px] border border-brand-border dark:border-line bg-white dark:bg-card">
              {f.icon}
            </div>
            <h3 className="mt-5 font-montserrat text-[17px] font-bold text-brand-maroon">
              {f.title}
            </h3>
